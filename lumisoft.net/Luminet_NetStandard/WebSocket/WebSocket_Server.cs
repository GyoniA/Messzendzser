...
{
    public class WebSocket_Server<T> : IDisposable where T : WebSocket_ServerSession, new()
    {
        ...
        private class WebSocket_Acceptor : IDisposable
        {
            ...
            public WebSocket_Acceptor(Socket socket)
            {
                if (socket == null)
                {
                    throw new ArgumentNullException("socket");
        ...
        private List<WebSocket_Acceptor> m_pConnectionAcceptors = null;
        private List<ListeningPoint> m_pListeningPoints = null;
        private WebSocket_SessionCollection<WebSocket_ServerSession> m_pSessions = null;
        ...
        public void Stop()
        {
            ...
            foreach (WebSocket_Acceptor acceptor in m_pConnectionAcceptors.ToArray())
            {
                try
                {
                    acceptor.Dispose();
            ...
            try
            {
                ...
                foreach (IPBindInfo bind in m_pBindings)
                {
                    try
                    {
                        ...
                        for (int i = 0; i < 10; i++)
                        {
                            WebSocket_Acceptor acceptor = new WebSocket_Server<T>.WebSocket_Acceptor(socket);
            ...
            try
            {
                ...
                else
                {
                    session.Disonnected += new EventHandler(delegate (object sender, EventArgs e) {
                        m_pSessions.Remove((WebSocket_ServerSession)sender);
                    });
        ...
        /// <exception cref="ObjectDisposedException">Is raised when this object is disposed and this property is accessed.</exception>
        /// <exception cref="InvalidOperationException">Is raised when TCP server is not running and this property is accesed.</exception>
        public WebSocket_SessionCollection<WebSocket_ServerSession> Sessions
        {
            get
            {
                if (m_IsDisposed)
        ...
        /// </summary>
        public event EventHandler<WebSocket_ServerSessionEventArgs<T>> SessionCreated = null;
        ...
        private void OnSessionCreated(T session)
        {
            ...
            {
                this.SessionCreated(this, new WebSocket_ServerSessionEventArgs<T>(this, session));
            }
        }
